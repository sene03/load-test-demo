 ...
 *
 * 결과물:
 * - results/html/phase1/phase1-rps{N}-{timestamp}.html
 * - results/json/phase1/phase1-rps{N}-{timestamp}-summary.json
 */
...
http.setResponseCallback(http.expectedStatuses(200, 409, 410));
...
const serverErrRate = new Rate('server_error_rate'); // 500~599: 서버 내부 오류
...
export function handleSummary(data) {
  ...
  const htmlPath     = `results/html/phase1/${fileNameBase}.html`;
  const jsonPath     = `results/json/phase1/${fileNameBase}-summary.json`;
  ...
  console.log(`
  === Phase 1 Load Test (Baseline) [${displayTime}] ===
  ...
  return {
    ...
    [htmlPath]: htmlReport(data, { title: `Phase 1 Test (Baseline): ${TARGET_RPS} RPS (${displayTime})` }),
    [jsonPath]: JSON.stringify({
      metadata: { phase: 1, rps: TARGET_RPS, time: displayTime, vu_max: vuMax },
      throughput: {
