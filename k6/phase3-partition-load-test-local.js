/**
 * Phase 3 Partition Test — Spring Boot + MySQL + Redis + Kafka (3 partitions + concurrency=3)
 *
 * 비교 대상: phase3-load-test-local.js (단일 파티션 + concurrency=1)
 *
 * 실행 방법:
 ...
 *
 * 결과물:
 * - results/html/phase3-partition/phase3p-rps{N}-{timestamp}.html
 * - results/json/phase3-partition/phase3p-rps{N}-{timestamp}-summary.json
 */
...
const TARGET_RPS = parseInt(__ENV.TARGET_RPS || '12000');
...
const redisErrorRate = new Rate('redis_error_rate');
const timeoutRate    = new Rate('timeout_rate');
const serverErrRate  = new Rate('server_error_rate');
...
export function handleSummary(data) {
  ...
  const fileNameBase = `phase3p-rps${TARGET_RPS}-${timestamp}`;
  const htmlPath     = `results/html/phase3-partition/${fileNameBase}.html`;
  const jsonPath     = `results/json/phase3-partition/${fileNameBase}-summary.json`;
  ...
  console.log(`
  === Phase 3 Partition Test (3 partitions + concurrency=3) [${displayTime}] ===
  Target RPS        : ${TARGET_RPS}
  Actual RPS (측정구간): ${measureRps.toFixed(2)} /s
  Total Requests    : ${totalReqs}
  Dropped           : ${dropped} (${droppedRate.toFixed(2)}/s)
  avg waiting       : ${reqParts.waiting} ms
  ...
  return {
    ...
    [htmlPath]: htmlReport(data, { title: `Phase 3 Partition Test: ${TARGET_RPS} RPS (${displayTime})` }),
    [jsonPath]: JSON.stringify({
      metadata: { phase: '3-partition', rps: TARGET_RPS, time: displayTime, vu_max: vuMax },
      throughput: {
